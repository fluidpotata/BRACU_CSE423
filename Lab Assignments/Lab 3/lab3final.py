...
def fallingBalls():
    global ballpos, pause, gameon, ballspeed, point, life, lastball, shooterPos
    if gameon and not pause:
        next = True
        ...
        if len(ballpos) < 5 and next:
            ...
            x = random.randint(-400+size, 400-size)
            for i in ballpos:
                d = ((i[1]-x)**2 + (i[2]-(400-size))**2)**(0.5)
                if d<=(i[0]+size+5):
                    check = False
            if check:
                ballpos.append([size, x, 400-size, True, random.randint(6, 11) / 10, random.randint(6, 11) / 10, random.randint(6, 11) / 10])
    glutPostRedisplay()
...
def shootingMove():
    global shootingballs, ballspeed, ballpos, point, bullspeed, pause, bullets, gameon, shooterSpeed
    for i in shootingballs:
        i[1] += bullspeed
        if i[1] >= 400:
        ...
        if (state == GLUT_DOWN):
            if (50 <= x <= 100) and (925 <= W_Height - y <= 975):
                if not gameon:
                    ...
                    # shooterSpeed = 8
                    ballpos = [
                        [random.randint(25, 40), random.randint(-375, 375), 375, True, random.randint(6, 11) / 10,
                         random.randint(6, 11) / 10, random.randint(6, 11) / 10]]
                    shootingballs = []
                    # bullspeed = 12
                    life = 3
                    bullets = 3
